...
# Saving to a file
set term png
set output "all_vcpus_credit2.png"
...
# Envelop all the vcpus of Domain-0 together and plot all of them in a single chart
plot "new_xentrace_credit2.txt" using 2:(strcol(1) eq "0v0" ? $3 : 1/0) title "VCPU-0 of Domain-0" with points pointtype 5, \
    "new_xentrace_credit2.txt" using 2:(strcol(1) eq "0v1" ? $3 : 1/0) title "VCPU-1 of Domain-0" with points pointtype 6, \
     "new_xentrace_credit2.txt" using 2:(strcol(1) eq "0v2" ? $3 : 1/0) title "VCPU-2 of Domain-0" with points pointtype 7, \
     "new_xentrace_credit2.txt" using 2:(strcol(1) eq "0v3" ? $3 : 1/0) title "VCPU-3 of Domain-0" with points pointtype 8
